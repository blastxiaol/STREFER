...
class OwnCenterPointBBoxCoder(BaseBBoxCoder):
    ...
    def decode(self,
        ...
        xs = xs.view(
            batch, self.max_num,
            1) * self.out_size_factor/(2**(up_factor)) * self.voxel_size[0] + self.pc_range[0]
        ys = ys.view(
            batch, self.max_num,
            1) * self.out_size_factor/(2**(up_factor)) * self.voxel_size[1] + self.pc_range[1]
